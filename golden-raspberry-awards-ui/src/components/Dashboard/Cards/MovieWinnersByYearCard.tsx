    ...
    <Card title="List movie winners by year" className='card'>
        ...
        <div style={{display: 'flex', marginBottom: '10px'}}>
          ...
          <span className="p-float-label" style={{marginRight: '3px', display: 'inline', width: '100%'}} >
            <InputNumber inputId="minmax-buttons" useGrouping={false} value={searchYear} onValueChange={(e: any) => setSearchYear(e.value)} showButtons min={1800} max={currentYear} style={{width: '100%'}}/>
            <label htmlFor="number-input">Search by year</label>
          </span>

          <Button label="" type="submit" icon="pi pi-search" onClick={(event: any) => { if (onSearchWinnersByYear) onSearchWinnersByYear(event, searchYear) }}/>
