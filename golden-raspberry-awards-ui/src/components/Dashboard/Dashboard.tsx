...
import { getMaxMinInterval, findMovieWinnersByYear, getMultipleWinners, getStudiosWithWinCount } from '../../services/Dashboard.service';
    ...
    const loadData = () => {
      ...
      .then(([multipleWinnersData, maxMinIntervalData, studiosWithWinCountData]) => {
        ...
        if (studiosWithWinCountData) {
          const top3 = studiosWithWinCountData.studios.sort((studioA, studioB) => studioB.winCount - studioA.winCount).slice(0, 3);
          setTop3studiosWithWinCount({ ...studiosWithWinCountData, studios: top3 });
        }
      })
